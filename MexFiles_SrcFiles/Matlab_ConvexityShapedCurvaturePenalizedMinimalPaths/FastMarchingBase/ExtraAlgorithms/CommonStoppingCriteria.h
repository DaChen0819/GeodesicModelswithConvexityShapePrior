...
CommonStoppingCriteria<T>::PostProcess(IndexCRef acceptedIndex){
    ...
    if(stopWhenAnyAccepted.find(acceptedIndex)!=stopWhenAnyAccepted.end()){
        PointType acceptedPoint = this->pFM->dom.PointFromIndex(acceptedIndex);
        nearestTip=acceptedIndex;
        
        std::vector<PointType> acceptedPoints;
        acceptedPoints.push_back(acceptedPoint);
        pFMI->ExportGeodesics("",acceptedPoints);
        return Decision::kTerminate;
    }
...
CommonStoppingCriteria<T>::Setup(HFMI* that){
    ...
    if(io.HasField("stopWhenAnyAccepted")){
        ...
        for(const auto & p : targets) {
            PointType pt=param.ADim(p);
            for(int k=0;k<Dimension-HFM::Traits::nStencilDependencies;k++)
                pt[k]+=0.5;
            IndexType index = dom.IndexFromPoint(pt);
    ...
    if(io.HasField("stopWhenAllAccepted")){
        ...
        for(const auto & p : targets) {
            ...
            if(dom.Periodize(index,index).IsValid())
                stopWhenAllAccepted.insert(index);
            else Msg() << "Warning : target " << p << " is out of range.";
        }
    }
...
CommonStoppingCriteria<T>::Finally(HFMI* that){
    if(pFM!=nullptr){
        that->io.template Set<ScalarType>("nAccepted",(ScalarType)nAccepted);
        const auto & dom = that->pFM->dom;
        const auto & param = that->stencil.Param();
        if(that->io.HasField("stopWhenAnyAccepted")){
            std::vector<PointType> firstReachedPoint;
//            PointType pt=param.ReDim(dom.PointFromIndex(nearestTip));
            PointType pt=param.ReDim(PointType::CastCoordinates(nearestTip));
            firstReachedPoint.push_back(pt);
            that->io.template SetVector("firstAcceptedTip",firstReachedPoint);
        }
