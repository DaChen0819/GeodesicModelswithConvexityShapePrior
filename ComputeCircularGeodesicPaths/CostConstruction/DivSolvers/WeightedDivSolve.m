 function u = WeightedDivSolve(shapeGradient,weightedDistance,tube,gridScale)
 
    weightedDistance(tube<0.5)=0;
    assert(ismatrix(shapeGradient));
    assert(all(size(shapeGradient)==size(weightedDistance)));
    [uh,uv]=WeightedDivSolveRaw(shapeGradient,weightedDistance);
