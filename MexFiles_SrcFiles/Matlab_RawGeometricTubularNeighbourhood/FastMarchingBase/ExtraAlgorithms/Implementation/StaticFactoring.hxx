...
ComputeFactor(HFMI * that){
	...
	const std::string choice= io.GetString("factoringPointChoice", fm.order<3 ? "Seed" : "Both");
...
SetSeeds(HFMI * that) {
	...
	if(io.HasField("seeds")){
		...
		for(auto & p : seedPoints)
            p=stencil.Param().ADim(p);
		if(compute){
			if(seedPoints.size()==1){
                seed=seedPoints[0];
            }
			else{
                ExceptionMacro("Exactly one seed point must be specified for computing "
