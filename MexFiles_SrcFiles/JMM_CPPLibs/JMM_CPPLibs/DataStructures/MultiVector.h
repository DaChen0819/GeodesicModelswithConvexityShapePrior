...
MultiVector<TV,TS>::insert(const std::vector<KeyValuePair> & data, SizeType smax){
	...
	assert(slast>=size());
	splits.insert(splits.end(), slast-size(), (int)values.size());

	for(const auto [s,val] : data){
		if(slast!=s){
			splits.insert(splits.end(), s-slast, (int)values.size());
			slast = s;
		}
	...
	if(smax==0) return;
	assert(smax>=slast);
	splits.insert(splits.end(),smax-slast, (int)values.size());
}
