...
protected:
    bool exportEuclideanLengths=false;
    bool estimateEuclideanLengths=false;
...
Setup(HFMI * that){
    auto & io = that->io;
    if(io.HasField("exportEuclideanLengths"))
        exportEuclideanLengths=io.template Get<ScalarType>("exportEuclideanLengths")>0.5;
    else
        exportEuclideanLengths=false;
    
    if(io.HasField("estimateEuclideanLengths"))
        estimateEuclideanLengths=io.template Get<ScalarType>("estimateEuclideanLengths")>0.5;
    else
        estimateEuclideanLengths=false;
