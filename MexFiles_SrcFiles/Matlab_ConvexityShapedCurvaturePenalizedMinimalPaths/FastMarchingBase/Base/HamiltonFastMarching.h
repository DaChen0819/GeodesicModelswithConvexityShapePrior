...
struct HamiltonFastMarching {
    ...
    //------ used for circular geodesic detection--------//
    Array<ScalarType,Dimension-1> physicalActiveRegion; //mask in the physical space.
    Array<ScalarType,Dimension-1> scribbleRegion; // tubular regions surrounding the scribbles.
    Array<ScalarType,Dimension-1> taggedAdaptiveCutRegion ;// tubular regions surrounding the adaptive cut. Each point in this tubular region has a unique label.
    Array<ScalarType,Dimension>   totalCurvature;
    
    bool simplicity_TotalCurvature=false;
    bool simplicity_AngularObstacle=false;
    
    typedef typename LinearAlgebra::Point<DiscreteType, Dimension-1> PhysicalIndexType;
    std::set<PhysicalIndexType,typename PhysicalIndexType::LexicographicCompare> scribbles;
    std::set<PhysicalIndexType,typename PhysicalIndexType::LexicographicCompare> adaptiveCut;
    
    ScalarType angularScale=0.0;
    std::string parameterizationOrder; // the curve is clociwise ordered or counter-clockwised.
//    bool offsetRemoval=true;
    bool extraCrossingCheck = true;
    int angularBound,angularUpBound,angularBottomBound=0;
    int numOriens=60;
    //-------------------------------------------------------------//
...
//protected:
    ...
    //---------------- used for circular geodesic detection --------------------------
//    bool ConvexSimpleGeodesic(IndexType);
    bool ConvexSimpleGeodesic_AugularObstacles(IndexType,OffsetType);
    bool ConvexSimpleGeodesic_TotalCurvature(IndexType,VectorType&);
    bool AdaptiveCutCrossingDetection(IndexType,IndexType) const;
    bool ScribblesCrossingDetection(IndexType,IndexType) const;
    //--------------------------------------------------------------------------------
