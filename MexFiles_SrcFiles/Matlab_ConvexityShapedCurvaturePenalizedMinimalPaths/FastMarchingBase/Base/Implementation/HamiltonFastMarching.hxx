...
void HamiltonFastMarching<T>::RunInit(){
    ...
    numOriens=(int)this->values.dims[2];
    angularScale = (2.0*Traits::mathPi)/(ScalarType)numOriens;
    ...
    for(const auto & [index,value] : seeds){
        ...
        angularBound=(int)index[Dimension-1];
    }
    angularUpBound=angularBound+numOriens;
    angularBottomBound=angularBound-numOriens;
    
}

/*
 Apply the Proposition 4 for ensuring the physical projection curves to be simple.
 */
template<typename T>
bool HamiltonFastMarching<T>::ConvexSimpleGeodesic_AugularObstacles(IndexType acceptedIndex,OffsetType offset){
    
    if(!simplicity_AngularObstacle)
        return false;
    ...
    int updatedAngularPos=(int)updated.index[Dimension-1];
    int acceptedAngularPos=(int)acceptedIndex[Dimension-1];
    
    
//    if(seedIndexSet.find(acceptedIndex)!= seedIndexSet.end())
//        return false;
    
    if(updatedAngularPos==angularBound || updatedAngularPos==angularBottomBound || updatedAngularPos==angularUpBound)
        return false;
    
//    if(angularBound==acceptedAngularPos && angularBound==updatedAngularPos)
//        return false;
    VectorType vecAccepted, vecRef, vecBound;
    vecAccepted[0]=cos(angularScale*(ScalarType)(acceptedAngularPos));
    vecAccepted[1]=sin(angularScale*(ScalarType)(acceptedAngularPos));
    vecRef[0]=cos(angularScale*(ScalarType)(angularBound));
    vecRef[1]=sin(angularScale*(ScalarType)(angularBound));
    vecBound[0]=cos(angularScale*(ScalarType)(angularBound+12));
    vecBound[1]=sin(angularScale*(ScalarType)(angularBound+12));
    ScalarType scalarProduct=vecAccepted[0]*vecRef[0]+vecAccepted[1]*vecRef[1];
    ScalarType boundProd=vecRef[0]*vecBound[0]+vecRef[1]*vecBound[1];
    if(scalarProduct<boundProd)
        return false;
    
    int angularLength=(int)offset[Dimension-1];
    int currentAngularPos=acceptedAngularPos;
    if(parameterizationOrder == "Clockwise"){
        int count=1;
        while(count<=std::abs(angularLength)){
            currentAngularPos=std::floor(acceptedAngularPos+count);
            if((int)currentAngularPos==angularBound||(int)currentAngularPos==angularUpBound||(int)currentAngularPos==angularBottomBound)
                return true;
            count++;
        }
    }
    
    if(parameterizationOrder == "AntiClockwise"){
        int count=1;
        while(count<=std::abs(angularLength)){
            currentAngularPos=std::floor(acceptedAngularPos-count);
            if((int)currentAngularPos==angularBound||(int)currentAngularPos==angularUpBound||(int)currentAngularPos==angularBottomBound)
...
/*
 An alternative way for ensuring the simplicity of the physical projection curves:
 directly estimating  the total curvature of the physical projection curves of the geodesic paths.
 This is done during the fast marching front propagation and remove the points whose total curvature is higher than 2pi.
 See the last paragraph of Section III of the TPAMI paper.
 */
template<typename T>
bool HamiltonFastMarching<T>::ConvexSimpleGeodesic_TotalCurvature(IndexType acceptedIndex, VectorType& flowSum){
    
    if(!simplicity_TotalCurvature)
        return false;
    ...
    curvature=(flowSum[Dimension-1]*angularScale);
...
bool HamiltonFastMarching<T>::RunOnce(){
    ...
    VectorType realFlow;
    if(simplicity_TotalCurvature && ConvexSimpleGeodesic_TotalCurvature(accepted.index,realFlow))
        return queue.empty();
    
    const auto offsets = stencilData.ReversedOffsets(accepted);
    for(OffsetCRef offset : offsets){
//        if(offsetRemoval){
//            if(parameterizationOrder == "Clockwise" && offset[Dimension-1]>0.0)
//                continue;
//            if(parameterizationOrder == "AntiClockwise" && offset[Dimension-1]<0.0)
//                continue;
//        }
        if(simplicity_AngularObstacle && ConvexSimpleGeodesic_AugularObstacles(accepted.index,offset))
            continue;
...
HamiltonFastMarching<T>::ConditionalUpdate(IndexCRef acceptedIndex,
    ...
    // check if the current updated points are inside the given mask.
    ...
    if(AdaptiveCutCrossingDetection(acceptedIndex,updated.index))
        return;
...
const -> RecomputeType {
	...
	(OffsetType offset, int & ord) -> ScalarType {
        ...
        // ----------------- used for circular geodesic only -----------------//
        if(extraCrossingCheck){
            if(ScribblesCrossingDetection(updatedIndex,acceptedIndex)){
                ord=0;
                return -Traits::Infinity();
            }

            if(AdaptiveCutCrossingDetection(updatedIndex,acceptedIndex)){
                ord=0;
                return -Traits::Infinity();
            }
        }
...
AdaptiveCutCrossingDetection(IndexType acceptedIndex,IndexType updatedIndex) const{
