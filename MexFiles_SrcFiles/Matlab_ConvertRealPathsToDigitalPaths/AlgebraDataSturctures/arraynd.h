...
template <class T> class CArray3D : protected CArray1D<T>
{
  ...
  public:
	...
	template <class S> CArray3D<T> Convolve(const CArray1D<CTriplet<int> > &, const CArray1D<S> &) const;
...
// Convolve with respect to a given neighborhood
template <class T> template <class S> CArray3D<T> CArray3D<T>::Convolve(const CArray1D<CTriplet<int> > &arrayNeighbors, const CArray1D<S> &arrayMask) const
{
	CArray3D<T> arrayRes;
	int iNeighbor;
	S *pMask;
    ...
    CTriplet<int> p, p2, minBound, maxBound;
    const CTriplet<int> *pNeighbor;
	CArray1D<int> arrayNeighborsOffsets(arrayNeighbors.GetSize());
