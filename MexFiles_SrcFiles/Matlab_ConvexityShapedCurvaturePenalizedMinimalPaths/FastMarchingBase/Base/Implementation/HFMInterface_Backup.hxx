...
Run_SetupSolver() {
    ...
    pFM->offsetRemoval=io.template Get<ScalarType>("order",1.)>0.5;
    
    if(io.HasField("useExtraCrossingCheck")){
        pFM->useExtraCrossingCheck=(io.template Get<ScalarType>("useExtraCrossingCheck")>0.5);
    }
    else
        pFM->useExtraCrossingCheck=true;
    
    if(io.HasField("useSimplicity")){
        pFM->useSimplicity=(io.template Get<ScalarType>("useSimplicity")>0.5);
    }
    else
        pFM->useSimplicity=true;
...
ExportGeodesics(KeyCRef suffix, const std::vector<PointType>& tips){
    ...
    if(checkReachTip){
        ...
        int numGeodesics=geodesics.size();
...
Run_ExtractGeodesics() {
    ...
    if(io.HasField("stopWhenAllAccepted") && !io.HasField("prescribedValues")){
        valuesAtTips.clear();
        std::vector<PointType> tips = io.GetVector<PointType>("stopWhenAllAccepted");
        for(PointType & tip : tips){
            PointType tipTem = stencil.Param().ADim(tip);
    ...
    if(io.HasField("stopWhenAllAccepted") && io.HasField("prescribedValues")){
        std::vector<PointType> tips = io.GetVector<PointType>("stopWhenAllAccepted");
        std::vector<ScalarType> prescribedValues=io.GetVector<ScalarType>("prescribedValues");
        IndexType  minTipIdx;
        ScalarType minVal=std::numeric_limits<ScalarType>::infinity();
        for(int k=0;k<tips.size();k++){
            PointType tipTem = tips[k];
            tipTem=stencil.Param().ADim(tipTem);
            for(int j=0; j<Dimension-HFM::Traits::nStencilDependencies;j++)
                tipTem[j]+=0.1;
            IndexType tipIdx=this->pFM->dom.IndexFromPoint(tipTem);
            if(!pFM->dom.PeriodizeNoBase(tipIdx).IsValid()){
                WarnMsg() << "GetStencil error : point " << tipTem << "is out of range.\n";
            ...
            if((pFM->values(tipIdx)+prescribedValues.at(k)) < minVal){
                minVal=pFM->values(tipIdx)+prescribedValues.at(k);
                minTipIdx=tipIdx;
            }
        }
        std::vector<PointType> tipMinimal;
        PointType minTip=this->pFM->dom.PointFromIndex(minTipIdx);
        tipMinimal.push_back(minTip);
        ExportGeodesics("",tipMinimal);
        return;
    }
