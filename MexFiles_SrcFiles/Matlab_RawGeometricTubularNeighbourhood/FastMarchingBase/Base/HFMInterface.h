...
#include "ExtraAlgorithms/LabelBasedStoppingCriteria.h"
#include "ExtraAlgorithms/LabelRetrieval.h"

template<typename T> void HFMInterface<T>::
Run_SetupExtraAlgorithms(){
    SetupSingleAlgorithm<LabelBasedStoppingCriteria<T> >();
    SetupSingleAlgorithm<LabelRetrieval<T> >();
    ...
    if(!pTime->ImplementIn(pFM.get()))
        pTime.reset();
    
    if(pVar && pTime)
        pVar->pCurrentTime = &(pTime->currentTime);
}
