...
Run_SetupSolver() {
    ...
    if(io.HasField("ExtraCrossingCheck")){
        pFM->extraCrossingCheck=(io.template Get<ScalarType>("ExtraCrossingCheck")>0.5);
    }
    else
        pFM->extraCrossingCheck=true;
    
    if(io.HasField("Simplicity_TotalCurvature"))
        pFM->simplicity_TotalCurvature=io.template Get<ScalarType>("Simplicity_TotalCurvature")>0.5;
    else
        pFM->simplicity_TotalCurvature=true; //default
       
    if(io.HasField("Simplicity_AngularObstacle"))
        pFM->simplicity_AngularObstacle=io.template Get<ScalarType>("Simplicity_AngularObstacle")>0.5;
    else
        pFM->simplicity_AngularObstacle=false; //default
    
    if(!pFM->simplicity_TotalCurvature && !pFM->simplicity_AngularObstacle)
        std::cout<<"No procedure is applied for curve simplicity."<<std::endl;
    
    int verbosity= (int)io.Get<ScalarType>("verbosity",0.0);
    if (verbosity==1){
        if(pFM->simplicity_TotalCurvature && !pFM->simplicity_AngularObstacle)
            std::cout<< "The total curvature of the physical projection curve is used for curve simplicity."<<std::endl;
        else if(!pFM->simplicity_TotalCurvature && pFM->simplicity_AngularObstacle)
            std::cout<< "The angular obstacle is used for curve simplicity."<<std::endl;
    }
    
    if(io.HasField("parameterizationOrder")){
        const std::string parameterizationOrder=io.GetString("parameterizationOrder");
        if(parameterizationOrder!= "Clockwise" && parameterizationOrder!="AntiClockwise" && parameterizationOrder!="None"){
            ...
            pFM->parameterizationOrder="Clockwise";
        }
        else pFM->parameterizationOrder=parameterizationOrder;
    }
    else pFM->parameterizationOrder="Clockwise";
    
    typedef typename LinearAlgebra::Point<ScalarType,   Dimension-1> PhysicalPointType;
    typedef typename LinearAlgebra::Point<DiscreteType, Dimension-1> PhysicalIndexType;
    
    if(io.HasField("physicalActiveRegion"))
        pFM->physicalActiveRegion = io.GetArray<ScalarType, Dimension-1>("physicalActiveRegion");
    else if(io.HasField("physicalMask"))
        pFM->physicalActiveRegion = io.GetArray<ScalarType, Dimension-1>("physicalMask");
    
    if(io.HasField("adaptiveCut")){
        std::vector<PhysicalPointType> adaptiveCut = io.GetVector<PhysicalPointType>("adaptiveCut");
        for(PhysicalPointType p : adaptiveCut){
            PointType pt;
            pt.fill(0.0);
            for(int i=0;i<Dimension-HFM::Traits::nStencilDependencies;i++)
                pt[i]=p[i];
            pt=stencil.Param().ADim(pt);
            for(int k=0; k<Dimension-HFM::Traits::nStencilDependencies;k++)
                pt[k]+=0.1;
            IndexType idx=pFM->dom.IndexFromPoint(pt);
            PhysicalIndexType PhysicalIndex;
            for(int k=0;k<Dimension-HFM::Traits::nStencilDependencies;k++)
                PhysicalIndex[k]=idx[k];
            pFM->adaptiveCut.insert(PhysicalIndex);
        }
    }
    else
        ExceptionMacro("Error : adaptiveCut must be provided!");
    
    if(io.HasField("adaptiveCutRegionTagged")){
        pFM->taggedAdaptiveCutRegion = io.GetArray<ScalarType, Dimension-1>("adaptiveCutRegionTagged");
    }
    else
        ExceptionMacro("Error : adaptiveCutRegionTagged must be provided!");
    
    
    if(io.HasField("scribbles")){
        std::vector<PhysicalPointType> scribbles = io.GetVector<PhysicalPointType>("scribbles");
        for(PhysicalPointType p : scribbles){
            PointType pt;
            pt.fill(0.0);
            for(int k=0; k<Dimension-HFM::Traits::nStencilDependencies;k++)
                pt[k]=p[k];
            ...
            for(int k=0;k<Dimension-HFM::Traits::nStencilDependencies;k++)
                PhysicalIndex[k]=idx[k];
            pFM->scribbles.insert(PhysicalIndex);
        }
        if(io.HasField("scribbleRegion"))
            pFM->scribbleRegion = io.GetArray<ScalarType, Dimension-1>("scribbleRegion");
        else
            ExceptionMacro("Error : scribbleRegion must be provided!");
    }
...
ExportGeodesics(KeyCRef suffix, const std::vector<PointType>& tips){
    ...
    if(pGeodesicSolver==nullptr){
        ...
        HFM::DomainType::periodizeUsesBase ? "ODE" : io.GetString("geodesicSolver", "Discrete");
    ...
    // check if geodesic curves reach the tips.
    bool checkReachTip=false;
    if(io.HasField("checkReachTip")){
        checkReachTip=io.template Get<ScalarType>("checkReachTip")>0.5;
    }
    std::vector<std::vector<PointType>> newGeodesics;
    if(checkReachTip){
        const auto& adaptiveCutRegion = io.GetArray<ScalarType, Dimension-1>("adaptiveCutRegionTagged");
        int numGeodesics=(int)geodesics.size();
        for(int k=0;k<numGeodesics;k++){
            std::vector<PointType> geo =geodesics.at(k);
            PointType tip=tips.at(k);
            IndexType indexTip=this->pFM->dom.IndexFromPoint(tip);
            IndexType indexGeoBack=this->pFM->dom.IndexFromPoint(geo.back());
            PhysicalIndexType physicalIndexTip,physicalIndexGeoBack;
            VectorType vec;
            vec.fill(0.0);
            for (int i=0;i<Dimension-1;i++){
                physicalIndexTip[i]=indexTip[i];
                physicalIndexGeoBack[i]=indexGeoBack[i];
                vec[i]=(ScalarType)(indexTip[i]-indexGeoBack[i]);
            }
            int lalelTip=(int)adaptiveCutRegion(physicalIndexTip);
            VectorType tangent;
            tangent.fill(0);
            for(int i=0;i<Dimension-1;i++)
                tangent[i]=geo.back()[i]-geo.at(geo.size()-2)[i];
            tangent=tangent.Normalized();
            bool flag=false;
            PointType lastGeoPoint=geo.back();
            int lalelGeoBack=(int)adaptiveCutRegion(physicalIndexGeoBack);
            while(!flag){
                lalelGeoBack=(int)adaptiveCutRegion(physicalIndexGeoBack);
                if(lalelTip!=lalelGeoBack && vec.Norm()<4){
                    flag=true;
                    break;
                }
                std::vector<PointType> newTips;
                PointType newTip=lastGeoPoint+tangent;
                newTips.push_back(newTip);
                std::vector<std::vector<PointType>> newGeos=pGeodesicSolver->Run(this,newTips);
                std::vector<PointType> newGeo=newGeos.front();
                for(PointType pt : newGeo)
                    geo.push_back(pt);
                ...
                for(int i=0;i<Dimension-1;i++)
                    tangent[i]=geo.back()[i]-geo.at(geo.size()-2)[i];
                tangent=tangent.Normalized();
                lastGeoPoint=newGeo.back();
                indexGeoBack=this->pFM->dom.IndexFromPoint(newGeo.back());
            ...
            newGeodesics.push_back(geo);
        }
    }
    else{
        for(std::vector<PointType> geo:geodesics)
            newGeodesics.push_back(geo);
    }
    
    
    std::vector<ScalarType> geodesicLengths;
       for(const auto & geo : newGeodesics) {geodesicLengths.push_back((ScalarType)geo.size());}
       ...
       for(const auto & geo : newGeodesics)
           for(const PointType & p : geo){
               PointType newPoint=p;
               for(int k=0;k<Dimension-HFM::Traits::nStencilDependencies;k++) //HFM::Traits::nStencilDependencies
                   newPoint[k]-=0.5;
...
Run_ExtractGeodesics() {
    ...
    if(io.HasField("stopWhenAnyAccepted") && io.HasField("stopWhenAllAccepted"))
        WarnMsg() << "stopWhenAnyAccepted and stopWhenAllAccepted cannot be used simultaneously.\n";

    if(io.HasField("stopWhenAnyAccepted") && !io.HasField("stopWhenAllAccepted") && !io.HasField("Tips"))
        return;
    
    std::vector<ScalarType> valuesAtTips;
    if(io.HasField("tips") && pFM->simplicity_TotalCurvature){
        valuesAtTips.clear();
        ...
        for(PointType & tip : tips){
            ...
            for(int k=0; k<Dimension-HFM::Traits::nStencilDependencies;k++)
                tipTem[k]+=0.1;
            IndexType tipIdx=this->pFM->dom.IndexFromPoint(tipTem);
            if(!pFM->dom.PeriodizeNoBase(tipIdx).IsValid()){
                WarnMsg() << "GetStencil error : point " << tipTem << "is out of range.\n";
                continue;
            }
            valuesAtTips.push_back(pFM->values(tipIdx));
            tip=this->pFM->dom.PointFromIndex(tipIdx);
        }
        ExportGeodesics("",tips);
        io.SetVector("valuesAtTips", valuesAtTips);
        return;
    }
       
    if(io.HasField("stopWhenAllAccepted") && pFM->simplicity_TotalCurvature){
        valuesAtTips.clear();
        std::vector<PointType> tips = io.GetVector<PointType>("stopWhenAllAccepted");
        for(PointType & tip : tips){
            PointType tipTem = stencil.Param().ADim(tip);
            for(int k=0; k<Dimension-HFM::Traits::nStencilDependencies;k++)
                tipTem[k]+=0.1;
...
Run_ExportData() {
	...
	if(io.Get<ScalarType>("exportActiveOffsets",0.,3)) {
        ...
        for(DiscreteType i=0; i<flow.size(); ++i){
			...
			for(int j=(int)discreteFlow.size(); j<(int)discreteFlow.max_size(); ++j) {elem[j]=0;}
        }
